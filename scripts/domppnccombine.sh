...
while [ $alldone == false ]; do
	...
	if [[ ${inst[$i]} == "false" ]]; then
	    cdt=$($datefunc incdatemid "$caltype $icdt ${inc}")
	    cdt=${cdt// /}
	else
	    cdt=$($datefunc incdate "$caltype $icdt ${inc}")
	    cdt=${cdt// /}
	fi
	...
	cdtchk=$($datefunc incdate "$caltype $cdt ${incchk}")
	cdtchk=${cdtchk// /}
	...
	if [[ -e $cffile ]] && [[ ! -e $fdfile ]]; then
	    echo skipping $ffdfile
    	    icdt=$($datefunc incdate "$caltype $icdt ${inc}")
    	    dateoffile[$i]=${icdt// /}
	    continue
	fi
